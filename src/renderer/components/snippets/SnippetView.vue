<template>
  <div v-click-outside="onClickOutside" class="snippet-view" >
    <div class="snippet-view__title">
      <div class="snippet-view__header">
        <AppInput ref="input" v-model="name" ghost class="snippet-name" />
        <div class="snippet-view__actions">
          <div v-if="isMarkdown" 
            class="snippet-view__actions-item eye" :class="{ active: app.markdownPreview}">
            <AppIcon name="eye" @click="onMarkdownPreview" />
          </div>
          <div class="snippet-view__actions-item">
            <AppIcon name="clipboard" @click="onCopySnippet" />
          </div>
          <div
            class="snippet-view__actions-item" @click="onAddTab" >
            <AppIcon name="plus" />
          </div>
        </div>
      </div>
      ...
      <div class="snippet-desc">              
        <AppInput type="textarea" ref="input" ghost class="description" rows="3" v-model="description"></AppInput>
      </div> 
    ...
    <div class="snippet-view__body">
      <SnippetTabs v-model="activeTab" :tabs="fragments"
        :width="snippetWidth"
        @tab:edit="onEditTab"
        @tab:delete="onDeleteTab" >
        <SnippetTabsPane v-for="(i, index) in fragments"
          :key="i.index"
          :label="i.label"
          :index="index"
          :split="false" >
          <MonacoEditor v-show="!app.markdownPreview || !isMarkdown"
            v-model="i.value"
            :language="i.language"
            :is-tabs="fragments.length > 1"
            @change:lang="onChangeLanguage($event, index)"
            @change:layout="onChangeLayout" />
          <MarkdownPreview v-if="i.language === 'markdown' && i.value"
            :model="i.value"
            :is-tabs="fragments.length > 1" />
        </SnippetTabsPane>
      </SnippetTabs>
...
export default {
  ...
  computed: {
    ...
    description: {
      get () {
        return this.localSnippet ? this.localSnippet.description : ''
      },
      set (v) {
        this.localSnippet.description = v
      }
    },
